...
function create_generation_row($user_id, $keyword, $group_id = null) {
    ...
    if ($group_id) {
        ...
        $gRes  = supabase_call('GET', '/rest/v1/content_groups?id=eq.' . urlencode($group_id) . '&select=client_id,site_id,publish_site_id');
        ...
        if (!empty($gData[0]['publish_site_id'])) $row['publish_site_id'] = $gData[0]['publish_site_id'];
    }
